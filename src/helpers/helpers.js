export async function fetchData(urls, options) {
    try {
    const [movieResponse, tvResponse] = await Promise.all([
        fetch(urls.urlMovie, options),
        fetch(urls.urlTV, options),
    ]);
...
export function mezclarItems(object, noItems = null) {
  ...
  const maxLength = Math.max(...extraerItems.map((items) => items.length));
